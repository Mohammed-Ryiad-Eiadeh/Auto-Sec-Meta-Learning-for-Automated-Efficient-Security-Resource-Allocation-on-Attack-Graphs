...
process_graph <- function(file_path) {
  ...
  # Define the GAT model with two hidden layers
  gat_model <- nn_module(
    initialize = function(input_dim, hidden_dim1, hidden_dim2, output_dim, num_heads) {
      self$attn_layer1 <- nn_multihead_attention(embed_dim = hidden_dim1, num_heads = num_heads)
      self$attn_layer2 <- nn_multihead_attention(embed_dim = hidden_dim2, num_heads = num_heads)
    ...
    forward = function(features, adjacency_matrix) {
      # Ensure the features are in the correct shape for multihead attention
      features <- features$view(c(vcount(g), 1, 1))  # Shape: (num_nodes, 1, 1) for batch_size=1 and feature_dim=1

      # First layer: Apply linear transformation, followed by attention and ReLU activation
      h <- torch_relu(self$linear1(features))

      # Multi-head attention - the first part of the returned tuple is the attention output
      attention_output <- self$attn_layer1(h, h, h)[[1]]  # The first element is the output of the attention mechanism

      # Use the attention output for the next layer
      h <- attention_output

      # Second layer: Apply linear transformation, followed by attention and ReLU activation
      h <- torch_relu(self$linear2(h))

      # Apply the second multi-head attention layer
      attention_output <- self$attn_layer2(h, h, h)[[1]]  # The first element is the output of the attention mechanism

      # Use the attention output for the output layer
      h <- attention_output
  ...
  model <- gat_model(input_dim = 1, hidden_dim1 = 64, hidden_dim2 = 128, output_dim = 256, num_heads = 4)
...
for (graph_file in graph_files) {  # Use 'graph_file' to refer to each individual file
  ...
  # Check if the result is valid before accessing the embedding
  if (length(result) >= 2 && !is.null(result[[1]])) {
    ...
    print(paste("Invalid result for file:", graph_file))
  }
}
...
if (length(all_embeddings) > 0) {
  ...
  print(paste("Total training time for all graphs: ", total_time, " seconds"))
  print(paste("Average training time per graph: ", total_time / length(graph_files), " seconds"))
} else {
