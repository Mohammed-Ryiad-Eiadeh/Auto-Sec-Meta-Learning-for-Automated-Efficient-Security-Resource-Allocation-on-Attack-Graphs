...
process_graph <- function(file_path) {
  ...
  # Define the GNN model with two hidden layers
  gcn_model <- nn_module(
    initialize = function(input_dim, hidden_dim1, hidden_dim2, output_dim) {
      self$layer1 <- nn_linear(input_dim, hidden_dim1)
      self$layer2 <- nn_linear(hidden_dim1, hidden_dim2)
      self$layer3 <- nn_linear(hidden_dim2, output_dim)
    },

    forward = function(features, adjacency_matrix) {
      # First hidden layer
      h <- torch_relu(self$layer1(features))

      # Second hidden layer
      h <- torch_relu(self$layer2(h))

      # Apply adjacency matrix (message passing)
      h <- torch_mm(adjacency_matrix, h)
  ...
  model <- gcn_model(input_dim = 1, hidden_dim1 = 64, hidden_dim2 = 128, output_dim = 256)
